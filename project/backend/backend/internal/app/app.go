...
func (a *App) setupRouter(h *handlers.Handlers, svc *services.Services) *gin.Engine {
	...
	v1 := router.Group("/api/v1")
	{
		...
		// Farm routes (Phase 1 Domain Hierarchy)
		farms := v1.Group("/farms")
		farms.Use(middleware.AuthMiddleware(authService))
		{
			farms.POST("", h.Farm.CreateFarm)
			farms.GET("", h.Farm.ListFarms)
			farms.GET("/:id", h.Farm.GetFarm)
			farms.POST("/:farm_id/units", h.Farm.CreateProductionUnit)
			farms.GET("/:farm_id/units", h.Farm.ListProductionUnits)
		}

		// Production Unit routes
		units := v1.Group("/units")
		units.Use(middleware.AuthMiddleware(authService))
		{
			units.GET("/:id", h.Farm.GetProductionUnit)
			units.POST("/cohorts", h.Farm.CreateCohort)
			units.POST("/assign-device", h.Farm.AssignDevice)
			units.POST("/sampling", h.Farm.RecordSampling)
			units.POST("/mortality", h.Farm.RecordMortality)
			units.GET("/:id/readings", h.Multisensor.GetLatestReadings)
			units.GET("/:id/readings/history", h.Multisensor.GetParameterHistory)
		}

		// AquaDoc Clinical AI & Knowledge Base routes (Phase 2)
		aquadoc := v1.Group("/aquadoc")
		aquadoc.Use(middleware.AuthMiddleware(authService))
		{
			aquadoc.POST("/chat", h.AquaDoc.Chat)
			aquadoc.GET("/conversations", h.AquaDoc.ListConversations)
			aquadoc.GET("/conversations/:id", h.AquaDoc.GetConversation)
		}

		// AquaTwin Digital Twin routes (Phase 4)
		twin := v1.Group("/twin")
		twin.Use(middleware.AuthMiddleware(authService))
		{
			twin.GET("/:unit_id/state", h.AquaTwin.GetTwinState)
			twin.GET("/:unit_id/timeline", h.AquaTwin.GetTimeline)
			twin.POST("/:unit_id/snapshot", h.AquaTwin.ForceSnapshot)
		}

		// Unified Alerts & Decision Interlocks routes (Phase 5)
		alerts := v1.Group("/alerts")
		alerts.Use(middleware.AuthMiddleware(authService))
		{
			alerts.GET("", h.AquaTwin.ListAlerts)
			alerts.POST("/:id/resolve", h.AquaTwin.ResolveAlert)
		}

		// AquaPredict Biometric Forecasting routes (Phase 7)
		predict := v1.Group("/predict")
		predict.Use(middleware.AuthMiddleware(authService))
		{
			predict.GET("/growth", h.AquaPredict.PredictGrowth)
		}

		// Research Data Export routes (Phase 9)
		export := v1.Group("/export")
		export.Use(middleware.AuthMiddleware(authService))
		{
			export.GET("/research/json", h.ResearchExport.ExportJSON)
			export.GET("/research/csv", h.ResearchExport.ExportCSV)
		}
		...
		monitoring.Use(middleware.AuthMiddleware(authService))
		{
			...
			monitoring.POST("/multisensor", h.Multisensor.IngestTelemetry) // Normalized Multisensor Telemetry
		...
		vision.Use(middleware.AuthMiddleware(authService))
		{
			...
			vision.POST("/observations", h.AquaPredict.RecordObservation)
			vision.GET("/units/:id/latest", h.AquaPredict.GetLatestVisionObservation)
