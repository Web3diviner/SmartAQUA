...
import { launchGoogleOAuthPopup, isValidGoogleClientId } from '@/utils/googleAuth'
...
export const AuthModal: React.FC = () => {
  ...
  const handleGoogleClick = async () => {
    ...
    if (isValidGoogleClientId(googleClientId)) {
      const opened = await launchGoogleOAuthPopup(
        ...
        (err) => {
          setLoading(false)
          console.info('Google popup notice:', err)
      ...
      // Smooth inline Google account sign-in
      setLoading(false)
      setShowDirectGmail(true)
    }
  }
  ...
  const handleDirectGmailSubmit = async (e: React.FormEvent) => {
    ...
    const raw = directGmail.trim()
    const formattedEmail = raw.includes('@') ? raw : `${raw}@gmail.com`
    ...
    <div className="modal-backdrop" onClick={closeAuthModal}>
      <div className="modal-card auth-modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="auth-header-copy">
          ...
          <p>Sign in with your Google account or create a manual farm profile to continue.</p>
        </div>
        ...
        {!showDirectGmail ? (
          ...
          <form className="custom-gmail-form" onSubmit={handleDirectGmailSubmit}>
            <div className="form-group">
              <label>Enter your Google / Gmail address *</label>
              <input
                type="email"
                required
            ...
            <div className="google-chooser-footer">
              <button
                ...
                onClick={() => setShowDirectGmail(false)}
              >
                ← Back
              </button>
            </div>
        ...
        {authMode === 'signup' && (
          <form className="auth-form" onSubmit={handleSignUpSubmit}>
            ...
            <button type="submit" className="button button--primary auth-submit-btn" disabled={loading}>
              {loading ? 'Creating Account…' : 'Create Farm Account (Sign Up)'}
            </button>
          </form>
        )}
